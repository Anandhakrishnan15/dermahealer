...
export async function sendRemindersJob() {
    try {
        ...
        for (const item of followUps) {
            try {
                const email = item?.patientDetails?.email?.trim();
                ...
                const treatment = item?.appointment?.treatment || item?.treatment || "";
                ...
                try {
                    if (phone) {
                        ...
                        if (
                            ...
                            !processedPhones.has(formattedPhone)
                        ) {
                            ...
                            console.log(`Invalid phone for ${name}`);
                        }
                    }
                } catch (err) {
                    console.error(`WhatsApp failed for ${name}:`, err.message);
                ...
                console.error("Reminder item error:", err);
            }
        }
